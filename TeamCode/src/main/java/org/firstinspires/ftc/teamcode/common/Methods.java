...
//            int clickTarget = robot.lift.getCurrentPosition();
            int clickTarget = Range.clip(robot.lift.getCurrentPosition() - (int) (gamepad2.left_stick_y * 300), MIN_OUTTAKE_CLICKS, MAX_OUTTAKE_CLICKS);

//            if (gamepad1.dpad_down){
//                clickTarget = Range.clip(robot.lift.getCurrentPosition() - 300, MIN_OUTTAKE_CLICKS, MAX_OUTTAKE_CLICKS);
...
//            else if (highOuttake.is(Button.State.HELD)) clickTarget = HIGH_OUTTAKE;
        ...
        public void robotBaseIntakeLoop() {
            if (gamepad1.right_trigger >= 0.1) {
                robot.pivot.setPosition(Range.clip(robot.pivot.getPosition() - (0.1 * gamepad1.right_trigger), Constants.CLAW_ROT_UP, Constants.CLAW_ROT_DOWN));
                rot_claw_state = IntakeState.CLAW_ROT_AMBIGUOUS;
            } else if (gamepad1.left_trigger >= 0.1) {
                robot.pivot.setPosition(Range.clip(robot.pivot.getPosition() + (0.1 * gamepad1.left_trigger), Constants.CLAW_ROT_UP, Constants.CLAW_ROT_DOWN));
                rot_claw_state = IntakeState.CLAW_ROT_AMBIGUOUS;
            }
...
//                }
            ...
            if (right_claw.is(Button.State.TAP)) {
                if (right_claw_state == IntakeState.RIGHT_CLOSED && rot_claw_state != IntakeState.CLAW_ROT_UP) { //only open claw if our claw is down/ambiguous. Don't want to open our claw when
                    robot.rc.setPosition(Constants.RIGHT_CLAW_RELEASE);
                    right_claw_state = IntakeState.RIGHT_OPEN;
                } else if (right_claw_state == IntakeState.RIGHT_OPEN) {
                    robot.rc.setPosition(Constants.RIGHT_CLAW_HOLD);
            ...
            if (rot_claw.is(Button.State.TAP)) {
                if (rot_claw_state == IntakeState.CLAW_ROT_AMBIGUOUS) {
                    ...
                    robot.pivot.setPosition(Constants.CLAW_ROT_UP);
                    rot_claw_state = IntakeState.CLAW_ROT_UP;
                } else if (rot_claw_state == IntakeState.CLAW_ROT_UP) {
                    robot.pivot.setPosition(Constants.CLAW_ROT_DOWN);
